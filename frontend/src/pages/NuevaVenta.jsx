...
export default function NuevaVenta() {
  ...
  const enviar = async () => {
    if (!cliente.nombre.trim()) return show("Ingresa el nombre del cliente.", "error");
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div className="card card-pad">
            <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 14 }}>Datos del cliente</h3>
            <div className="form-grid">
              ...
              <div className="field">
                ...
                <input
                  value={cliente.ruc}
                  onChange={e => setCliente(c => ({ ...c, ruc: e.target.value }))}
        ...
        {/* Derecha — resumen */}
        <div className="card card-pad" style={{ height: "fit-content", position: "sticky", top: 24 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 16 }}>Resumen</h3>

          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
            {items.filter(i => i.codigo).map((it, idx) => {
              const p = getProducto(it.codigo);
              if (!p) return null;
              return (
                <div key={idx} style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                  <span style={{ color: "var(--muted)" }}>{p.nombre} ×{it.cantidad}</span>
                  <span className="mono">S/ {(p.precio * +it.cantidad).toFixed(2)}</span>
                </div>
              );
            })}
          </div>
          ...
          <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12, display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 16, fontWeight: 800, marginTop: 4, color: "var(--green)" }}>
              <span>TOTAL</span>
