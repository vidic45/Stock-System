...
export default function Inventario() {
  ...
  const abrirAgregar = () => { setForm(VACIO); setModal("agregar"); };
  const abrirEditar  = p  => { setForm({ ...p }); setModal(p); };
  const cerrar = () => setModal(null);

  const guardar = async () => {
    if (!form.codigo || !form.nombre || form.precio === "" || form.stock === "" || form.minimo === "")
      return show("Completa todos los campos.", "error");

    setGuardando(true);
    const esNuevo = modal === "agregar";
    const url  = esNuevo ? `${API}/productos` : `${API}/productos/${modal.codigo}`;
  ...
  const eliminar = async (codigo) => {
    ...
    if (res.ok) { show("Producto eliminado."); cargar(); }
    ...
    <>
      ...
      <div className="card">
        <div className="card-pad section-header">
          <span style={{ fontWeight: 700, fontSize: 15 }}>Lista de productos</span>
          <button className="btn btn-primary" onClick={abrirAgregar}>+ Agregar producto</button>
        </div>
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {productos.length === 0 && (
                <tr><td colSpan={7} className="empty">Sin productos aún.</td></tr>
              )}
              {productos.map(p => (
                <tr key={p.codigo}>
                  ...
                  <td>
                    <span className={`badge ${p.estado}`}>
                      {p.estado === "ok" ? "✅ OK" : p.estado === "bajo" ? "⚠️ Bajo" : "❌ Agotado"}
                    </span>
                  </td>
                  <td>
                    <div style={{ display: "flex", gap: 6 }}>
                      <button className="btn btn-ghost btn-sm" onClick={() => abrirEditar(p)}>Editar</button>
                      <button className="btn btn-danger btn-sm" onClick={() => eliminar(p.codigo)}>Eliminar</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {modal && (
        <div className="modal-overlay" onClick={cerrar}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{modal === "agregar" ? "Agregar producto" : `Editar — ${modal.codigo}`}</h3>
